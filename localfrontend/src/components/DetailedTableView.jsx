...
const MismatchIcon = ({ size = 20, className = "" }) => (
    <svg
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
        viewBox="0 0 24 24"
    ...
    return (
        <div className={`detail-field-item ${isMismatched ? 'mismatched' : ''}`}>
            <div className="detail-field-header">
                ...
                <span className="detail-field-status">
                    {isMismatched ? <MismatchIcon /> : <CheckIcon />}
                </span>
            </div>
            <div className="detail-field-values">
                <div><span className="value-label">Input:</span> {data.input}</div>
                <div className={isMismatched ? 'mismatched-value' : ''}>
                    <span className="value-label">Extracted:</span> {data.extracted}
                </div>
            </div>
        </div>
    );
                ...
                {data.map(row => (
                    <React.Fragment key={row.id}>
                        <tr className={`overview-row ${expandedRowId === row.id ? 'expanded' : ''}`} onClick={() => toggleRow(row.id)}>
                            ...
                            <td>
                                <div className={`cell-toggle-icon ${expandedRowId === row.id ? 'expanded' : ''}`}>
                                    <ChevronDown />
                                </div>
                            </td>
                        </tr>
                                ...
                                <td colSpan="5">
                                    <div className="detail-row-content">
                                        <div className="detail-summary">
                                            <h4>Verification Details for {row.sourceFile}</h4>
                                            <p>Comparing data from the master CSV with content extracted from the document.</p>
                                        </div>
                                        <div className="detail-grid">
                                            {Object.keys(fieldLabels).map(fieldKey => (
                                                row[fieldKey] ? <DetailField key={fieldKey} label={fieldLabels[fieldKey]} data={row[fieldKey]} /> : null
                                            ))}
                                        </div>
                                    </div>
                                </td>
